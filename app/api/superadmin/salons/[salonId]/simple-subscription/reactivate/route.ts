...
export async function POST(
  ...
  try {
    ...
    const reason = body.reason?.trim() || "Reactivated by superadmin";
    ...
    await createAuditLog({
      ...
      entityType: "Salon",
      entityId: salonId,
      after: { subscriptionStatus: "active", reason },
      request,
    });

    return successResponse({ salonId, subscriptionStatus: "active", isActive: true }, "Salon reactivated.");
  } catch (error) {
    if (error instanceof SyntaxError) return errorResponse("Invalid JSON.", 400);
    return errorResponse("Unable to reactivate salon.", 500);
  }
}
