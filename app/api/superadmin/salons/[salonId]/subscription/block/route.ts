...
import { blockSalonAccess } from "@/src/lib/subscription-access-service";
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const reason = body.reason?.trim() || "Payment not received after grace period";

    const subscription = await blockSalonAccess(salonId, reason);
    ...
    await createAuditLog({
      ...
      after: { accessStatus: "access_blocked", reason },
      request,
    });

    return successResponse({ subscription }, "Salon subscription access blocked.");
  } catch {
    return errorResponse("Unable to block salon access.", 500);
  }
}
